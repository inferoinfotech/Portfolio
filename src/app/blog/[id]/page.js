...
export async function generateMetadata({ params }) {
  ...
  const fullContentString = Array.isArray(article.content)
    ...
    <>
      <section className="py-16 px-4 lg:px-8">
        <div className="mx-auto max-w-container px-4">
          ...
          <h1 className="text-3xl md:text-4xl xl:text-4xl pe-16 font-bold mb-6 leading-slug">
            {article.title}
          </h1>
          <p className="text-[#7C7C7C] text-md lg:text-xl mb-8">
            {article.date}
          </p>
          <div className="mb-8">
            {article.image.endsWith(".mp4") ? (
              <video
                ...
                muted
                playsInline
                className="w-full h-auto max-h-[490px] object-cover rounded-[40px]"
              />
            ) : (
              <img
                src={article.image}
          ...
          <div className="prose prose-invert text-lg xl:ps-8 xl:pt-8">
            {Array.isArray(article.content) ? (
              article.content.map((paragraph, index) => (
                <p
                  key={index}
                  className="text-black sm:text-[24px] lg:text-3xl xl:text-3xl my-8 leading-tight font-normal"
                >
                  {paragraph}
                </p>
              ))
            ) : (
              <p className="text-black text-base sm:text-lg md:text-xl lg:text-3xl xl:text-4xl font-normal">
                {article.content}
              </p>
            )}
          </div>
          <div className="flex flex-wrap flex-col lg:flex-row items-center justify-center mt-16 lg:mt-24 lg:mb-10 gap-6  cursor-pointer transition-transform duration-200 hover:scale-105">
            {/* Profile Image */}
            ...
            <div className="flex flex-col ">
              <h1 className="text-3xl lg:text-xl text-center lg:text-start font-medium text-black">
                Saleh Bilal
              </h1>
              <p className="text-lg lg:text-base text-center lg:text-start font-normal text-[#7A7A7AB2]">
